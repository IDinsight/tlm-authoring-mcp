...
Start by fetching the generation inputs — prefer `walk_document_section` for a single slot (the
first section of a document in full, every further one with `include:[]`), and `preview_generation`
