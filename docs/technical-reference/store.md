...
- **Authoring (curator+):** `add_nodes`, `create_edges`, `edit_nodes`, `move_node`, `delete_nodes`, `delete_edges` — see [Authoring verbs](#authoring-verbs) below. Each is two-phase (dry-run → token-only confirm) and audited on both writes and denials. Sequential edits accumulate on the SAME draft overlay and publish together atomically.
...
```
curator:  add_nodes([...]) / edit_nodes([...]) → dry-run: diff + token
...
- **`edit_nodes(items[{nodeId, [content, position, title, title_en, summary, properties]}])`** — edit ONE node's fields in place or MANY in one atomic draft edit. Each item carries its own values, so a batch can retitle one node and rewrite another's content; the same edit across N nodes is the same fields repeated per item. `content` replaces the load-bearing content (`Material.content`); `position` sets the ordinal among siblings (membership is the edge, so this **never cascades**); `title` / `title_en` set the display name (normalized to the node's `title` vs `text` field by its label, + `raw.description`); `properties` amends any other canonical LC prop under `raw.<key>`. All-or-nothing: any item error blocks the whole batch. It replaced the single-node `edit_node`, which had consolidated `reposition` + `set_content` and added title editing. Edit in place — never delete + re-add (that cascades the subtree, drops edges, and mints a new id).
- **`move_node(nodeId, toParentId, [via, position])`** — re-parent along one containment axis: detach every current parent edge on that axis, attach the new parent, set the ordinal there, all in ONE atomic draft edit. There are exactly two containment axes — `hasPart` (content) and `hasChild` (standards) — and the axis is **read off the graph**, whichever of the two the node actually hangs from (a node hanging from both falls back to its label's canonical edge), so a node's second axis (e.g. a maths lesson also scheduled under a week via `hasChild`) is left intact. Blocked, rather than guessed at: an unknown node or parent; a node made its own parent; a target parent that sits **inside** the node (that would cut the subtree out of the graph and leave it in a ring, reachable by nothing — generation included); a `via` that is not one of the two containment edges (detaching an alignment or reference edge would DELETE relationships, not move a node — rewire those with `delete_edges` + `create_edges`); and a node with no parent on the chosen axis (the error names the axis). It WARNS, without blocking, when the node has several parents on the axis and all of them are therefore detached. Doing this by hand would be `delete_edges` + `create_edges` + `edit_nodes` — three writes, any of which can be left half-done.
...
path its copied example uses, and `edit_nodes` / `move_node` write every path the node itself uses —
...
- **editable** — the live edit surface: `batch` (the `add_nodes` / `create_edges` tools + the per-kind `kindProperties` catalog + the response/idempotency controls), `structural.verbs` (`create_edges` / `delete_edges` / `delete_nodes`), and `recipes` (a mirror of the generic `RECIPES` registry — `edit_nodes` / `move_node`), all rendered straight from source.
