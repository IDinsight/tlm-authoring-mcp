...
- **`move_node(nodeId, toParentId, [via, position])`** — re-parent along one containment axis: detach every current parent edge on that axis, attach the new parent, set the ordinal there, all in ONE atomic draft edit. The axis defaults to the node's canonical containment edge (`hasPart` for content, `hasChild` for standards) and `via` overrides it; a node's second axis (e.g. a maths lesson also scheduled under a week via `hasChild`) is left intact. Blocked, rather than guessed at: an unknown node or parent, a node made its own parent, and a node with no parent on the chosen axis (the error names the axis). Doing this by hand would be `delete_edges` + `create_edges` + `edit_node` — three writes, any of which can be left half-done.
