...
6. **What its pages look like is data too — none of it is code.** A subject's documents are produced from three bags authored on its formatters, all validated at edit time and all changed without a deploy: `render` (geometry: page, type, block styles, images, budget), `layout` (structure: templates the composer fills from the graph, with placeholders and holes) and, in the same `layout` bag, `guide` (the **guide grammar**: how a section's assembly guide reads as page lines — which line prefixes print and in which voice, how a phrase of the repertoire is called and what it says, how a picture is named and marked, which inline assets a line may carry). `compose_section` fills the templates and compiles the guides; `lint_content` runs the formatter's declared `lintRules`; `render_document` lays the result out. The code knows block kinds, placeholders, three grammar motifs and matching; it knows no prefix, no phrase number, no glyph of any subject, and its compiler tests run on an invented grammar to keep it that way. So a new subject that wants its pages composed without a model authors a template list and, if its guides are line-structured, a grammar — on its formatter, through `edit_nodes` — and nothing under `src/` changes. See [`docs/design-notes/layout-templates.md`](../design-notes/layout-templates.md), [`formatter-render-spec.md`](../design-notes/formatter-render-spec.md) and [`data-driven-lint-rules.md`](../design-notes/data-driven-lint-rules.md); the CI-maths teacher sheet's bags are `backend/test/fixtures/senegal-fiche-layout.json` and `senegal-fiche-lint-rules.json`.
