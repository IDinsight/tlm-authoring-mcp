...
```
section        the DocumentSection node (its own position + assemblyGuide, if any — never its `metadata.journal`, the page's dated decision history, which `append_journal` writes and walk_graph reads)
