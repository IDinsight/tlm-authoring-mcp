...
export async function renderDocument(a: RenderArgs): Promise<Record<string, unknown>> {
  ...
  for (const [index, { variant, bytes }] of rendered.entries()) {
    ...
    const overlaps = measurement?.available
      ...
      : [];
    // The declared face, checked on the PDF: a substituted font changes every
    // line count that follows, silently. null = nothing declared, or no reader.
    const fontOk = measurement?.available ? fontAsDeclared(measurement.fonts, spec.spec.type?.family) : null;
    const gaps = measurement?.available
      ? measurement.perPage.flatMap((page) => page.gaps.map((gap) => ({ page: page.page, ...gap })))
      : [];
    ...
    files.push({
      ...
      // White nothing explains — a `clear` that nothing needed, usually. Each
      // one is a body line the page did not have to give.
      ...(gaps.length > 0 ? { gaps } : {}),
    });
  }
