...
export function editableSection(recipes: ReturnType<typeof recipesSection>) {
  return {
    ...
    documents: {
      tools: ["create_document", "add_section", "attach_image", "append_journal"],
      ...
      invariants: {
        ...
        attach_image: "records a picture as a `Material` under the Lesson or Activity it illustrates ONLY when its file is really in the bucket (or the caller says `commissioned:true`, ordering a picture whose file the illustrator will upload to that exact path) — a node pointing at a path that resolves to nothing by accident is a valid write and a picture that never renders; the node is canonical LC — its `identifier` is the file's URI, its `content` what the picture shows, read beside the activity's text — and `answerCells` + `answerCellsOf` record which cell(s) of a band are correct out of how many, so a media entry with mark:'answer' gets the teacher's check drawn by the server",
        append_journal: "adds ONE dated, signed entry to a document's or a section's decision journal (`metadata.journal`) by reading the field as it stands in the draft at the moment of writing and appending after it — an edit_nodes on that field rewrites it from the caller's copy, so two sessions on one document overwrite each other silently; a confirm whose base moved is refused",
      },
      note:
        "create_document / add_section replace the add_nodes + create_edges pair for authoring a document; attach_image replaces add_nodes for a picture; append_journal replaces edit_nodes for a journal entry. Use them for anything document-shaped; add_nodes stays the tool for curriculum nodes, where `alignTo` already carries the equivalent invariant atomically.",
    },
    ...
    render: {
      ...
      strict: true,
      note:
        "`lintRules` is a formatter's THIRD data bag beside `render` and `layout`: line rules as data ({id, where:'guide'|'content'|'page', match, require?, forbid?, maxChars?, unless?, sections?, message, fix?}) that lint_content runs over the guides, the covered curriculum or a composed page of the document the formatter governs, reporting `declared:<id>` — subject knowledge checked without a deploy. Validated at edit_nodes/add_nodes; the list replaces as a whole. " +
...
export function documentSection(actions: Actions) {
  return {
    ...
    note:
      "The block tree is what YOU compose and render_document lays out. `blocks` is an ordered list of: `table` ({rows: Cell[][]}, where a Cell is {blocks, style?, span?} — cells hold BLOCKS, so a banner and a grid of pictures are the same construct and tables nest), `line` ({runs, variant?, style?}, where a run is {text, style?, translate?: false — kept as written when the server derives another language, e.g. the parenthesis after a speech line} or {image: {media, role, aspectRatio, float?}}), `spacer` ({sizePt, leadingPt}), and `clear` ({} — the end of a wrap: what follows starts below the lowest floated picture; put one after the block a floated band anchors to when that block is shorter than the band, or the next band draws over it). `example` is a complete tree to copy from. Optional `media` carries each picture as {name, data} (data base64) OR {name, relPath} (a bucket path in this namespace's documents/ area the server resolves) OR {name, nodeId} (an attached picture) — exactly one per entry, so an illustrated document need not inline megabytes of base64 into the call; {name, nodeId, mark:'answer'} is the TEACHER'S COPY, the check drawn on the cell(s) the node records (attach_image's `answerCells`), never drawn by hand. " +
      "compose_section fills the formatter's `layout` templates from the graph and keeps the result under `treeRef`; a template's {kind:'unfilled'} block is a HOLE the model fills — `unfilled[]` names each with its guide and `insertAt`, the block path for an insert-before patch on that ref (fill from the last hole up, an insertion shifts the paths after it). A {kind:'guide'} block instead has the SERVER compile the section's assembly guide by the grammar the layout bag declares (`layout.guide`: which prefixes print and in which voice, the phrase repertoire, picture markers, inline assets) — `compiled[]` then says per section how many lines printed, stayed in the guide, pictures placed, and what could not be resolved (also under `problems`); read that page, fix the guide where the report says, do not recompose it. " +
...
export function discoverySection(actions: Actions) {
  return {
    ...
    perCallContext:
      "Every namespace-scoped tool takes an optional `context` ({workspace, grade, subject}) applying to THAT CALL ONLY, leaving the session's active context untouched: the graph reads (walk_graph / walk_document / walk_document_section / find_node / namespace_stats / get_standards / compose_section / check_document), the lexicon reads (get_terminology / translate), and every document tool (reconcile / list_documents / create_download_url / get_document_text / check_stale / create_upload_url / create_media_upload_url / log_generation / record_document_content). " +
    ...
    walkDocument: {
      ...
      // first — and the section spine, which nothing else provides, pages instead.
    ...
    walkDocumentSection: {
      ...
      include:
        "`include` (a subset of document/curriculum/routine/formatters/pictures; omit for all) drops the parts you already hold — the assembly guide, formatter stack, covered curriculum and the pictures attached to it are the SAME for every section of a document, so read it once with walk_document and then ask per section. The `covers` ids and `formatterStackOrder` always ride; the section's own node rides in full on the first page and as its skeleton on a continuation page; `omitted` names what you left out, so a missing `routine` is never read as a section that has none.",
      pagination:
        "Only the formatter stack pages. When it does not fit one response you get `formattersTruncated` + `nextCursor` + `stackNote`; a CONTINUATION page (fetched with `cursor`) carries ONLY the remaining formatters and is flagged `continued:true` — the section guide, document, routine and covered curriculum came on the first page, so re-sending them (a fixed pile bigger than the page budget) is skipped and the loop converges. The first page's own context is NOT paged, so on the fattest sections it alone exceeds the response cap and the read is refused with a remedy: pass detail:'skeleton' and/or include to shrink it.",
    },
    exportGraphView: {
    ...
    note:
      "walk_graph is the single generic traversal: a directional (out/in/both), edge- and label-filtered, PAGINATED BFS from any node — use it to discover the framework root, a course subtree, a standards spine, anything. It replaced get_course. Page with limit (default 50, max 500) + nextCursor; do not raise the limit to fit a big result. includeEdges defaults to FALSE — opt in only when you need the edges. `detail` chooses how much of each node rides: 'full' (default) is the whole property bag, 'skeleton' is identity + ordinal + kind with every authored-prose field dropped — the right choice for any STRUCTURAL question, because prose is where a page's bytes are (a ci/maths DocumentSection carries several KB of assemblyGuide, so a full page holds ~4 of them against a skeleton page's ~180). truncatedByLimit means more nodes remain on further pages; truncated means the depth cap hid deeper nodes; truncatedBySize means a response BYTE budget trimmed the page below `limit` (raising limit won't help — switch to detail:'skeleton', set includeEdges:false and narrow nodeTypes, then page via cursor; the `hint` field says so). slot:'published' (default) reads live; slot:'draft' inspects UNPUBLISHED staged edits (curator/approver only). walk_document is the document-side counterpart: given a TeachingLearningMaterial (TLM) root it returns that document's assemblyGuide, its Formatter/FormatterSpec rendering stack, its DocumentSection spine (in reading order, each section naming the `parent` it hangs under — sections nest), and the curriculum it renders (resolved by section spine or a TLM→covers→Course fallback — `scope` says which), so generation targets the document, not the raw curriculum. It self-bounds in tiers rather than refusing: the curriculum degrades first, then the TLM subtree (both readable per-section via walk_document_section), and the section spine — the one part nothing else provides — pages with `limit`/`cursor` instead of degrading. walk_document_section is the PER-PIECE generation entry, anchored on ONE DocumentSection — the node that already IS the document↔curriculum binding (it hangs under exactly one TLM and covers its curriculum), so it is the unit generation produces a document from, section by section: it returns the owning document, the curriculum the section renders, the routine that applies (nearest-wins, document-first — the section's own usesRoutine, else that of the sections it is nested in, else the owning TLM's, else the covered curriculum's ancestry, so a document-specific routine finally has a home), and the formatters (every stack on the section's own path: its own, its parent sections', and the TLM's doc-wide one). find_node turns a NAME into ids (« le chapitre 5 » → the matching nodes, accent- and case-insensitive, each with its containment path): it is how an id is obtained, so a user is never asked for one; when several match it says `ambiguous` and you ask which. namespace_stats is a cheap, argument-free orientation snapshot (node/edge counts, roots, draft state) — run it FIRST to see the shape of the graph before writing a walk; its `roots` (each with id + labels + description) surfaces the Course content roots AND the TLM document roots, so it replaced list_courses. export_graph_view returns a SELF-CONTAINED scoped slice (the containment subtree of a node) in the explorer's DisplayGraph shape — feed it to a self-contained HTML artifact to render the same interactive tree the live explorer shows; published slot only, self-bounded to the response cap.",
  };
}
