  ...
  server.registerTool(
    "duplicate_entry",
    {
      ...
      description:
        ...
        "GATED by the destination, and because it PUBLISHES it needs an APPROVER there. TWO-PHASE: the dry-run returns the diff + confirmationToken + mintedIdMap; confirm with confirm:true + the token, RE-SENDING the same `entryId`, `name` and `mintedIdMap` (a different — or omitted — `name` on the confirm produces a different copy and the token is rejected). " + PARKED_PAYLOAD_NOTE + " Then edit the copy in place with edit_node(nodeId, content, catalog).",
      inputSchema: {
        entryId: z.string().optional(),   // required on dry-run; omitted on token-only confirm
